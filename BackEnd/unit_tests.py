...
def test_spendings():
    ...
    assert (
        spending.value_eur == converted_values[0]
    ), "should be the value of converted_values[0] "
    assert (
        spending.value_usd == converted_values[1]
    ), "should be the value of converted_values[1] "
    assert (
        spending.value_gbp == converted_values[2]
    ), "should be the value of converted_values[2] "
    assert (
        spending.value_ron == converted_values[3]
    ), "should be the value of converted_values[3] "
