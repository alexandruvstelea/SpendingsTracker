...
class Spending(db.Model):
    ...
    def read(email_to_srch, start, end):
        spendings = Spending.query.filter(Spending.date.between(start, end)).filter(
            Spending.email == email_to_srch
        )
            ...
            if given_currency != currency:
                try:
                    converted_value = converter.convert(
                        given_currency, currency, value, date
                    )
    ...
    def average(email_to_srch, start, end):
        ...
        return {
            "average_eur": round(total_eur / len(spendings_list), 2),
            "average_usd": round(total_usd / len(spendings_list), 2),
            "average_gbp": round(total_gbp / len(spendings_list), 2),
            "average_ron": round(total_ron / len(spendings_list), 2),
        }
    ...
    def totalFilter(email_to_srch, start, end, category):
        spendings = (
            Spending.query.filter(Spending.date.between(start, end))
            .filter(Spending.email == email_to_srch)
            .filter(Spending.category == category)
        )
        spendings_schema = SpendingSchema(many=True)
        spendings_list = spendings_schema.dump(spendings)
        total_eur, total_usd, total_gbp, total_ron = 0, 0, 0, 0
    ...
    def accountData(email_to_srch, start, end):
        ...
        return {
            ...
            "total_gbp": round(total_gbp, 2),
            "total_ron": round(total_ron, 2),
            "big_eur": round(max(list_eur), 2),
            "big_usd": round(max(list_usd), 2),
            "big_gbp": round(max(list_gbp), 2),
