function registerUser() {
  ...
  fetch(url)
    ...
    .then(function (complete_response) {
      if (complete_response.response == "registered") goToSignIn();
      else console.log(complete_response);
    })
    .catch((err) => {
      console.log(err);
    });
...
function loginCredentials() {
  ...
  fetch(url, { method: "POST", credentials: "include" })
    ...
    .then(function (complete_response) {
      if (complete_response.response == "ok") goToIndex();
...
function verifyEmail() {
  const url =
    "http://127.0.0.1:5000/verifyemail?" +
    new URLSearchParams({
      email: document.getElementById("registerEmail").value,
    });
  fetch(url)
    ...
    .then(function (complete_response) {
      if (complete_response.response == "False") registerBtn();
      else errorMessageRegister("E-mail already exists");
    })
    .catch((err) => {
      console.log(err);
    });
