async function totalSpendingAccount() {
  ...
  const url =
    "http://127.0.0.1:5000/accountdata?" +
    new URLSearchParams({
  ...
  fetch(url)
    ...
    .then(function (complete_response) {
      ...
      if (values.total == 0)
        document.getElementById("all_spendings").innerText =
          values.total.toLocaleString("en-US") + " " + selectedCurrency;
      else
        document.getElementById("all_spendings").innerText =
          "-" + values.total.toLocaleString("en-US") + " " + selectedCurrency;
      if (values.biggest == 0)
        document.getElementById("biggest_spendings").innerText =
          values.biggest.toLocaleString("en-US") + " " + selectedCurrency;
      else
        document.getElementById("biggest_spendings").innerText =
          "-" + values.biggest.toLocaleString("en-US") + " " + selectedCurrency;
    })
