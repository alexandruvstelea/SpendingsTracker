...
function generateDays() {
  ...
  if (year % 4 == 0 && days == 28) days += 1;
  data = ``;
  for (i = 1; i <= days; i++) {
    data += `
      <div class="card-day">
        <button 
        onclick="fetchSpendings(${parseInt(i)}), setCurrentdDay(${parseInt(
      i
    )}), generateDays()"
...
function getBackgroundDays() {
  if (i == currentSelectedDay) return "#7CB9E8";
  if (i % 2 == 0) return "var(--primary-color)";
  else return "var(--secondary-color)";
}
