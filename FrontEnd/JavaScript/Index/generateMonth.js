...
function highlight() {
  ...
  for (var b = 0; b < radios.length; b++) {
    if (radios[b].checked) {
      document
        .getElementById(Object.keys(months)[b])
        .setAttribute("style", getBackground(1, b));
    } else
      document
        .getElementById(Object.keys(months)[b])
        .setAttribute("style", getBackground(0, b));
  }
}

function getBackground(status, number) {
  if (number == currentSelectedMonth) return "background: #7CB9E8;";
  if (status == 0) {
    if (number % 2 == 1) return "background: var(--secondary-color);";
    else return "background:var(--primary-color);";
  } else if (status == 1) return "background:#7CB9E8;";
}
